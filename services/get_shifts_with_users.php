<?php return function (\PDO $connection, int $id_shift_type): array {
    ...
    foreach (Tables\Shifts::select_all($connection, $id_shift_type) as $shift_day) {
        ...
        for ($shift_position = 1; $shift_position <= $shift_day['number']; $shift_position++) {
            ...
            $shift_day_list[$id_shift]['shifts'][$shift_time] = (isset($user_list[$shift_position]))? $user_list[$shift_position] : array();
        }
    }
