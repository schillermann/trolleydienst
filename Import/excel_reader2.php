...
class Spreadsheet_Excel_Reader {
	...
	function dump_test2($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
                include "../DBConnect.php";
		$AnzError = 0;
		$out="<table>";
		$out.="<tr><th>Zeilennr</th><th>Name</th><th>Ergebnis</th></tr>";
		...
		for($row=5;$row<=59;$row++) {
		  ...
		  $Vorname = nl2br($Vorname);
			

  		  $Nachname = $this->val($row,1,0);
		  if ($Nachname=='') 
		  { 
		    $Nachname = ""; 
  	          }
		  ...
		  $Nachname = nl2br($Nachname);

		  $Tel = $this->val($row,5,0);
		  if ($Tel=='')
		  {
		  	$Tel = "";
		  }
		  ...
		  {
		  	$mobil = htmlentities($mobil);
		  }
		  $mobil = nl2br($mobil);

		  $Mail = $this->val($row,7,0);
		  if ($Mail=='')
		  {
		  	$Mail = "";
		  }
		  ...
		  {
		  	$Trolley = htmlentities($Trolley);
		  }
		  $Trolley = nl2br($Trolley);

		  $Username=substr($Vorname,0,2).substr($Nachname,0,2);
    ...
    if (($Mail != '???') && ($Mail != ''))
    {
	  if (($Trolley == '1') || ($Trolley == '2'))
	  {
    		$out.="<tr><td>".$row."</td><td>".$Username." ".$Vorname." ".$Nachname." ".$Mail." ".$Tel." ".$mobil." ".$Trolley."</td>";
      ...
      $Betreff = "Zugangsdaten f�r jw-koelnost.de";
	  
      $text="Hallo ".$Vorname.",</br></br>";
	  $text.="willkommen bei <a href=\"http://www.jw-koelnost.de\">jw-koelnost.de</a>.<br>";
	  $text.="Hier wird das �ffentliche Zeugnisgeben einfach organisiert.<br><br>";
	  $text.="Deine Zugangsdaten lauten wie folgt:<br><br>";
	  $text.="<table border=0><tr><td>Benutzername:</td><td>".$Username."</td></tr>";
	  $text.="<tr><td>Passwort:</td><td>".$str_password."</td></tr></table>";
	  $text.="<br><br>";
	  $text.="Bitte �ndere dein Kennwort in dem Men�punkt Profil<br><br>";
	  $text.= 'Bei Anregungen und Problemen kannst du gerne ein Mail an ' . CONGREGATION_NAME . ' schreiben oder antworte einfach diese E-Mail.<br>';
	
	  $teilnehmernr=1;
	  $sqlcommandNewID="Select coalesce(Max(teilnehmernr),0) + 1 as teilnehmernr from teilnehmer";
	  $SQLResultNewID=mysql_query($sqlcommandNewID,$verbindung);
	  $rowNewID=mysql_fetch_object($SQLResultNewID);
	  if ($rowNewID->teilnehmernr > 0)
	  {
	  	$teilnehmernr = $rowNewID->teilnehmernr;
	  }
	  ...
	  $InsertCommand.="'".$teilnehmernr."',0,'".$Vorname."','";
	  $InsertCommand.=$Nachname."','".$Mail."','".$Username."','".md5($str_password)."',";
	  $InsertCommand.=$Info.",".$Trolley.",0,'".$Tel."','".$mobil."','K�ln-Ost','deutsch')";
	   
	  $SQLResult=mysql_query($InsertCommand,$verbindung);	  
	  if (!$SQLResult)
	  {
	  	$out.="<td>Teilnehmer konnte nicht importiert werden</td>";
	  	$AnzError = $AnzError + 1;
	  }
	  else
	  {
	  	$out.="<td>Teilnehmer wurde importiert</td>";
	  	SendMail($Mail,$Betreff,$text);
	  }
	  }
    }
      $out.="</tr>";
		...
		$out = "<h2>Import Verlauf</h2>Es gab ".$AnzError." Fehler<br>".$out;
